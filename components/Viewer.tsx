      ...
      <div className="w-[min(320px,80vw)] aspect-[9/19] max-h-[80vh] bg-navy rounded-[3rem] border-[8px] border-navy shadow-[0_0_60px_rgba(17,35,90,0.4)] relative pointer-events-auto overflow-hidden flex flex-col scale-90 sm:scale-100">
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar pt-12 px-6 pb-12 space-y-6 ${t === 'minimal' ? 'bg-white' : t === 'professional' ? 'bg-slate-50' : 'bg-gradient-to-br from-slate-50 to-offwhite'}`}>
          ...
          <div className="space-y-4">
            {blocks.map(block => (
              <div key={block.id} className="bg-white p-5 rounded-2xl shadow-[0_4px_20px_rgba(17,35,90,0.04)] border border-navy/5">
                <div className="flex items-center gap-3 mb-2"><div className="p-2 bg-cream rounded-lg text-petrol">{getIcon(block.type, block.buttonType)}</div><p className="font-black text-navy text-[10px] uppercase tracking-wider">{block.title || block.buttonType || 'Info'}</p></div>
                <p className="text-[11px] text-zinc-500 leading-relaxed font-medium">{block.content}</p>
                {block.type === 'image' && block.imageUrl && <div className="mt-4 rounded-xl overflow-hidden border border-navy/5"><img src={block.imageUrl} className="w-full h-32 object-cover" alt="Preview" /></div>}
              </div>
            ))}
          </div>
        </div>
...
const BaseModel = forwardRef<THREE.Mesh, { config: ModelConfig, showNFC?: boolean }>(({ config, showNFC }, ref) => {
  const geometry = useMemo(() => {
    ...
    if (config.baseType === 'circle') {
      shape.absarc(0, 0, 22.5, 0, Math.PI * 2, false);
    ...
    <group position={[config.logoPosX, 0, config.logoPosY]} scale={[config.logoScale, config.logoScale, config.logoScale]} rotation={[0, THREE.MathUtils.degToRad(config.logoRotation), 0]} ref={groupRef}>
      <group position={[-initialOffset.x, 0, -initialOffset.z]}>
        {logoElements.map((el, idx) => (
          <mesh key={idx} geometry={el.geo} castShadow><meshStandardMaterial color={el.color} /></mesh>
        ))}
      </group>
    </group>
  );
