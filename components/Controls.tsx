...
import { Box, Type, Trash2, Edit3, Link as LinkIcon, Image as ImageIcon, Sliders, Award, MessageCircle, Globe, ShoppingCart, Info, User, Mail, Phone, Loader2, Instagram, Utensils, Shield, Layout, Camera, Dumbbell, Heart, Palette, ArrowLeft, RefreshCw, Star, MapPin, Wifi, CreditCard, Briefcase, Zap, Sparkles, Home, Music, Hammer, Stethoscope, ChevronDown, ChevronUp, Calendar, Youtube, Video, Building2, Scissors, Palmtree } from 'lucide-react';
    ...
    return (
      <div className="space-y-8 animate-in slide-in-from-left">
        <div className="bg-navy p-5 rounded-3xl text-white shadow-xl flex items-center gap-4">
            ...
            <div>
               <span className="text-[8px] font-black uppercase text-action tracking-[0.2em]">3D Branding Editor</span>
               <p className="text-[10px] font-bold mt-0.5 opacity-70 leading-snug">Logo & Materialanpassung</p>
            </div>
        </div>

        <div className="space-y-3 px-1">
           <label className="text-[9px] font-black uppercase text-zinc-400 px-2 flex items-center justify-between">
              Branding (SVG) <span className="text-[7px] opacity-50">Auto-Scale</span>
           </label>
           <div className="relative h-32 rounded-[2rem] border-2 border-dashed border-navy/10 bg-white flex flex-col items-center justify-center gap-1 group shadow-sm overflow-hidden cursor-pointer hover:border-petrol/30 transition-all">
              <ImageIcon size={32} className="text-zinc-200" />
              <span className="text-[8px] font-black text-zinc-400 uppercase mt-2">Vektorgrafik hochladen</span>
              <input type="file" accept=".svg" onChange={onUpload} className="absolute inset-0 opacity-0 cursor-pointer" />
           </div>
        </div>

        <div className="space-y-3 px-1">
          <label className="text-[9px] font-black uppercase text-zinc-400 px-2">Farbauswahl</label>
        ...
        <div className="space-y-6 pt-4 border-t border-navy/5">
           <div className="space-y-2 px-2">
              <div className="flex justify-between text-[8px] font-black uppercase"><span className="opacity-50 tracking-widest">Logo Skalierung</span><span>{(config.logoScale * 100).toFixed(0)}%</span></div>
              <input type="range" min="0.1" max="2.0" step="0.01" value={config.logoScale} onChange={e => updateConfig('logoScale', parseFloat(e.target.value))} className="w-full accent-petrol" />
           </div>
           <div className="space-y-2 px-2">
              ...
              <input type="range" min="0.1" max="4.0" step="0.1" value={config.logoDepth} onChange={e => updateConfig('logoDepth', parseFloat(e.target.value))} className="w-full accent-petrol" />
           </div>
           <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2 px-2">
                 <div className="text-[8px] font-black uppercase opacity-50 tracking-widest">X-Position</div>
              ...
              <div className="space-y-2 px-2">
                 <div className="text-[8px] font-black uppercase opacity-50 tracking-widest">Y-Position</div>
    ...
    <div className="flex flex-col space-y-8 h-full">
      ...
      <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 px-1">
             {industryTemplates.map(tpl => (
               <button key={tpl.id} onClick={() => applyTemplate(tpl)} className={`p-4 bg-white border rounded-[2rem] flex flex-col items-start gap-3 transition-all active:scale-95 shadow-sm group hover:border-petrol/30 ${config.profileTitle === tpl.profileTitle ? 'border-petrol/20 ring-2 ring-petrol/5' : 'border-navy/5'}`}>
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[6px] font-bold text-zinc-400 uppercase mt-0.5 tracking-widest">Konfiguration laden</p>
                  </div>
               </button>
             ))}
          </div>
      ...
      <section className="bg-white border border-navy/5 p-6 rounded-[2.5rem] space-y-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
               <label className="text-[8px] font-black uppercase text-zinc-400 px-1 tracking-widest">Profil Logo</label>
               <div className="relative h-20 rounded-2xl border border-dashed border-navy/10 bg-cream flex items-center justify-center overflow-hidden cursor-pointer">
          ...
          <div className="space-y-8">
            {magicButtonsGroups.map((group) => (
              <div key={group.group} className="space-y-3">
                ...
                {group.items.map(btn => (
                    <button key={btn.id} onClick={() => addMagicButton(btn.type as MagicButtonType, btn.label)} className="p-3.5 bg-white border border-navy/5 rounded-[2rem] flex items-center gap-3 active:scale-95 shadow-sm group hover:border-petrol/30 transition-all">
      ...
      <section className="space-y-4 px-1 pb-44">
        ...
        <div className="space-y-3">
          {config.nfcBlocks.length === 0 && (
            <div className="text-center py-12 bg-white/50 rounded-[2.5rem] border border-dashed border-navy/10">
              <p className="text-[10px] font-black uppercase text-zinc-300 tracking-widest leading-loose">Noch keine Inhalte<br/>hinzugefügt</p>
            </div>
          )}
          {config.nfcBlocks.map((block) => (
            <div key={block.id} className="bg-white p-5 rounded-[2rem] border border-navy/5 flex items-center gap-4 shadow-sm group animate-in slide-in-from-right duration-500">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[7px] font-bold text-zinc-300 uppercase tracking-widest mt-1.5">{block.buttonType || 'Inhalt'}</p>
              </div>
